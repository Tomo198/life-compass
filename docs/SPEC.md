...
- 見直しメモ/レビュー
- 詳細積立シミュレーション
- 期間別の収入・生活費を置いた詳細取り崩しシミュレーション
- 退職後の生活費・年金・社会保険・税金・インフレを置いた老後生活プラン
- 家族/世帯モード

課金導入前は Coming soon の開発プレビューとして確認できる状態にし、無料版との機能境界を画面上で明示する。
...
| 予算・実績 | 月次の予算、実績、差額整理 | 履歴、前年比、レビューとの詳細連携 |
...
| 世帯イベント | 世帯全体の予定 | 本人、配偶者、子ども、親、世帯全体で対象者別に整理 |
| 診断 | 入力完了度の案内 | 家計、資産、目標、イベント、レビューの確認ポイント整理 |
| データ保存 | ブラウザ内保存、JSONバックアップ | ブラウザ内保存、JSONバックアップ、任意の暗号化クラウドバックアップを検討 |
| 課金 | なし | 月500円程度を想定 |
...
- 老後に限定せず、FIRE、セミリタイア、退職後の軽い労働、年金開始前のつなぎ期間などを扱う
- 働き方モードは固定せず、期間ごとの開始年齢、終了年齢、月収入、月生活費、年間特別支出を入力する
- 年間取り崩し額 = max(0, 期間ごとの年間生活費 + 年間特別支出 - 期間ごとの年間収入)
- 年末資産 = (前年資産 - 年間取り崩し額) × (1 + 想定利回り)
- 期間設定の間に空白年齢がある場合は、直前の期間条件を次の期間開始まで引き継ぐ
- 生活費と年間特別支出には物価上昇率を反映し、収入は入力額を固定して扱う
...
- 課金導入前はPro機能を開発プレビューとして試用できるようにし、Proバッジと料金表で境界を明示する
...
- 初期版では課金処理を実装しない
- 将来、Stripe Checkout後に `subscriptionTier` を更新する
- 課金状態はアプリ本体の表示制御と保存制限に使う
- Googleログインを導入する場合、ユーザー識別はメールアドレスではなくGoogleの安定した利用者識別子を使う
- サーバー側で持つ情報は、本人識別、契約状態、Stripe顧客ID、保存メタ情報など必要最小限にする
...
- Stripe秘密鍵、Google設定、署名検証用の値はCloudflare Secretsで管理し、リポジトリに保存しない
