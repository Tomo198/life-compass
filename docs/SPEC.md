...
8. シミュレーション
   - 基本資産推移
   - 年次キャッシュフロー（収入、支出、世帯年齢、資産見通し、家計の山場）
   - 簡易積立
   - 生活防衛資金
   - 将来見通し
...
11. Pro機能紹介
   - Coming soon
   - 月額590円（税込）を予定
   - 複数の見直しプラン比較、ライフプラン診断、レビューセンター、固定費見直しインパクト、積立・取り崩しのばらつき試算、老後生活プラン、暗号化クラウドバックアップ
...
- 見直しプラン
  - 「見直しプラン」と「比較結果」の2タブで、作成・編集と比較・採用を分ける
  - 見直しプランごとに家計、資産、積立、時期別収支、目標、ライフイベントを編集する
  - 基本プランを変更せずに年次グラフと比較表を確認する
  - 影響分析では順位付けや助言を行わず、現在プランから変更した前提、家計・資産への直接差、10年後・30年後を含む見通し全体の差を分けて表示する
  - 複数の前提変更がある場合、個々の直接差を将来資産差の内訳として断定しない
  - 選んだ見直しプランを基本プランへ採用し、採用前の基本条件は比較用の見直しプランとして自動保存する
  - 見直しプラン内で追加した時期別収支、目標、イベントも採用時に基本プランへ反映する
...
| 機能 | 無料版 | Pro版予定 |
| --- | --- | --- |
| ライフプラン保存 | 1プラン | 複数の見直しプラン保存 |
| 家計入力 | 基本収支 | 固定費見直し、見直しプランごとの差分 |
...
- 基本収支方式から詳細収支方式へ初めて切り替える際は、基本収支と重複期間の優先規則を含む時期別変更を年区間へ分割して引き継ぐ
- 方式を切り替えても反対側の入力データは削除せず、再切替時に再利用できるよう保持する
- Pro利用終了時に詳細収支方式が選択中でも詳細データは保持し、無料版で基本収支方式へ戻せるようにする
- 詳細収支項目は最大200件とし、対象者・種類・開始年・終了年・金額・メモを保存する
...
- シミュレーション内の独立タブで10年/30年を切り替え、グラフ、家計の山場、年次表を同じ選択年で連動する
