...
運営者テストの初期段階では、明示的な「共有へ保存」「共有から取得」から始めました。現在は利用者が端末ごとに明示的に有効化した場合だけ、ローカル優先の自動同期を行います。

自動同期の要件:

- 共有パスワードは平文で保存せず、取り出せない端末鍵で暗号化してIndexedDBへ保存する
- Googleログイン、世帯membership、ownerのPro権限は保存・取得のたびにWorkerで再確認する
- ブラウザ内保存を先に完了し、操作が落ち着いた後に暗号化して共有へ保存する
- 別端末の更新は30秒間隔と画面復帰時に確認する
- オフライン中はブラウザ内の変更を維持し、接続後に再試行する
- `expectedRevision`が一致しない場合は自動上書きしない
- 両方に変更がある場合は競合として停止し、選ばなかった内容を復旧用コピーへ残す
- ログインしただけでは有効にせず、端末ごとに共有パスワードで初回確認する
- 自動同期は自分専用かつ画面ロックを設定した端末に限定し、共有・公共端末では有効にしないよう案内する
- 端末の自動同期解除、世帯退出、世帯削除、membership失効時は端末内の共有設定を削除する

端末鍵は同一Originで実行される正規アプリから利用できるため、XSSへの防御を置き換えるものではありません。CSP、外部スクリプトの最小化、危険なHTML挿入の禁止を継続します。
...
R2削除が一時的に失敗した場合はD1上の追跡情報を残し、毎日のCronで再試行します。保存競合やD1障害で確定しなかった暗号文も `shared_plan_object_cleanup` で追跡し、参照中の版を除外したうえで削除します。
