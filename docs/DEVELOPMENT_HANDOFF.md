...
更新日: 2026-07-27
基準HEAD: `3ba9baf`
作業チェックポイント: 共同世帯同期・継続利用UX改善（未コミット）
...
- 共同利用者は、自動同期を有効にした共同世帯プランでのみ契約者の世帯内Pro権限を利用可能
- 共同利用者本人の個人用プランと個人クラウドバックアップへ世帯内Pro権限を波及させない

## 5. 共同世帯の自動同期

信頼済み端末の暗号化自動同期を実装し、2026-07-27に通信間隔と復元履歴の表示を調整しました。

- 端末ごとに共有パスワードを入力して明示的に有効化
- 通常入力は先にブラウザへ保存
- 操作が20秒落ち着いた後に暗号化して共有へ保存
- 表示中は60秒間隔、オンライン復帰時、画面復帰時に別端末の更新を確認
- `expectedRevision`による楽観的ロックを使用
- 同時編集は自動上書きせず、利用者が残す内容を選択
- 選ばなかった内容はブラウザの復旧用コピーへ保存
- 共有鍵更新、世帯退出、世帯削除、参加解除時は端末設定を無効化
- 共有履歴は現在のデータと復元用履歴に分け、日時を中心に表示
- 現在分を含む最新10件を保持し、超過分の削除失敗は日次処理でも再試行
...
2026-07-27の作業ツリーでの結果:

- ロジックテスト: 105件成功
- Worker APIテスト: 30件成功
- PC・スマホE2E: 54件成功
- 本番ビルド: 成功
- PC・スマホの実画面確認: 横はみ出しとコンソールエラーなし
- npm監査: 既知の脆弱性0件
...
1. 現在の未コミット差分を確認してコミット、GitHubへプッシュ
2. 本番環境で契約者と別Googleアカウントから、共同利用者の家族別入力と20秒同期を再確認
3. オフライン復帰、同時編集、参加解除、鍵更新、復元用履歴を再確認
4. リポジトリ全体の深いセキュリティ確認と依存関係監査を完了
5. 最大20見直しプラン、多数イベント、詳細収支の性能とFree降格を確認
...
- 今回の権限・同期間隔・UI改善を反映した後の実アカウント2人による本番再検証
- 任意の強固なプライバシーロック
- リポジトリ全体の深いセキュリティ確認
- 最大20見直しプランと多数入力時の性能確認
