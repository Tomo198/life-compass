    ...
    <>
      ...
      <LegalSection title="1. ライフプランデータ">
        <ul>
          ...
          <li>暗号化クラウドバックアップを提供する場合も、利用者が選んだときだけ使う任意機能とし、ブラウザ内で暗号化したデータだけを保存します。自動同期は行いません。</li>
      ...
      <LegalSection title="2. 取得する可能性がある情報">
        <p>Googleログインを利用した場合、Googleが発行する利用者識別子、メールアドレス、メール確認状態、Life Compass内部の利用者ID、ハッシュ化したログインセッション、有効期限、作成・更新日時を取得します。Pro版の提供開始後は、プラン区分、契約状態、更新日、Stripeの顧客・契約識別子を追加で取得する場合があります。カード番号などの決済情報はStripeが取り扱い、Life Compassのサーバーには保存しません。</p>
      </LegalSection>
      ...
      <LegalSection title="4. 外部サービス">
        <p>サイト配信、ログインセッション、利用者・契約情報の管理にCloudflareを利用し、暗号化クラウドバックアップを有効化した場合は暗号化済みデータをCloudflare R2へ保存します。GoogleログインにはGoogle Identity Services、決済にはStripeを利用する予定です。各社はサービス提供に必要な範囲で、アクセス情報、利用者情報、契約情報、決済情報を取り扱う場合があります。</p>
      </LegalSection>
