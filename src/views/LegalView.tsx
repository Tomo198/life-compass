  ...
  return (
    <>
      ...
      <LegalSection title="7. 照会・削除の依頼">
        <p>Googleログインの利用者情報は、ログイン後の設定画面から利用者自身で削除できます。削除するとLife Compass内部の利用者ID、Googleの利用者識別子、メールアドレス、関連セッション、および保存済みの暗号化クラウドバックアップが削除されます。ブラウザ内のライフプランデータは対象外で、データ管理画面またはブラウザ機能から削除できます。契約中または法令上保存が必要な情報がある場合の確認、訂正、削除についてはお問い合わせください。</p>
      </LegalSection>
    </>
  );
}
