    ...
    <>
      ...
      <LegalSection title="1. ライフプランデータ">
        <ul>
          ...
          <li>ライフプランデータの自動的なクラウド同期は行いません。通常の保存先は利用中のブラウザです。</li>
          <li>暗号化クラウドバックアップは、利用者が明示的に操作したときだけ使う任意機能です。ブラウザ内で暗号化したデータをCloudflare R2へ保存します。</li>
          <li>復旧パスワードはサーバーへ送信・保存しません。復旧パスワードを忘れた場合、運営者はバックアップを復元できません。</li>
          <li>JSONファイルの保管、共有、削除、別端末への移行は利用者自身で管理してください。</li>
        </ul>
      </LegalSection>
      <LegalSection title="2. 取得する可能性がある情報">
        <p>Googleログインを利用した場合、Googleが発行する利用者識別子、メールアドレス、メール確認状態、Life Compass内部の利用者ID、ハッシュ化したログインセッション、有効期限、作成・更新日時を取得します。Pro版の提供開始後は、プラン区分、契約状態、更新日、決済代行事業者が発行する顧客・契約識別子を追加で取得する場合があります。カード番号などの決済情報は決済代行事業者が取り扱い、Life Compassのサーバーには保存しません。</p>
      </LegalSection>
      ...
      <LegalSection title="4. 外部サービス">
        <p>サイト配信、ログインセッション、利用者・契約情報の管理にCloudflareを利用し、暗号化クラウドバックアップを有効化した場合は暗号化済みデータをCloudflare R2へ保存します。GoogleログインにはGoogle Identity Servicesを利用します。決済機能の提供開始後は、料金ページ等で案内する決済代行事業者を利用します。各社はサービス提供に必要な範囲で、アクセス情報、利用者情報、契約情報、決済情報を取り扱う場合があります。</p>
      </LegalSection>
      ...
      <LegalSection title="6. 保存期間・安全管理">
        <p>ログインセッションの有効期間は最大30日とし、ログアウト時に失効させます。期限切れまたは失効済みのセッション情報は定期的に削除します。利用者・契約・問い合わせ情報は、利用目的、法令上の保存義務、紛争対応に必要な期間だけ保持します。不正アクセス、漏えい、改ざんなどを防ぐため、アクセス制御、セッションのハッシュ化、秘密情報の分離など合理的な安全管理措置を講じます。</p>
      </LegalSection>
      <LegalSection title="7. 照会・削除の依頼">
        <p>Googleログインの利用者情報は、ログイン後の設定画面から利用者自身で削除できます。削除するとLife Compass内部の利用者ID、Googleの利用者識別子、メールアドレス、関連セッション、および保存済みの暗号化クラウドバックアップが削除されます。ブラウザ内のライフプランデータは対象外で、バックアップ・復元画面またはブラウザ機能から削除できます。契約中または法令上保存が必要な情報がある場合の確認、訂正、削除についてはお問い合わせください。</p>
      </LegalSection>
    </>
  );
}
  ...
  return (
    <>
      ...
      <LegalSection title="解約方法と反映時期">
        <ul>
          <li>アプリ内または決済サービスの契約管理画面から、いつでも解約手続きができます。</li>
      ...
      <LegalSection title="返金の申請">
        <p>決済日、決済サービスの領収書または決済記録に記載された情報、登録メールアドレス、申請理由を添えて、決済から7日以内を目安にお問い合わせください。カード番号全体やセキュリティコードは送信しないでください。</p>
      </LegalSection>
      <LegalSection title="解約後のデータ">
        <p>ブラウザ内に保存されたライフプランデータは、解約によって自動削除されません。Pro機能で作成した内容は残りますが、契約期間終了後はPro機能からの閲覧・編集が制限される場合があります。暗号化クラウドバックアップは新規保存できなくなりますが、ログイン済み本人は保存済みデータの復元と削除を行えます。事前にJSONバックアップも作成してください。</p>
      </LegalSection>
    </>
  );
}
